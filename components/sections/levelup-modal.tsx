...
import SubclassSelectionModal from '@/components/character-creation/subclass-selection-modal';
...
function SubclassScreen({ char, selectedSubclass, onSelect, onNext }: {
    ...
    return (
        <div style={{ animation: 'lu-fade-in 0.4s ease-out' }}>
            <SubclassSelectionModal
                characterClass={char.class}
                selectedSubclass={selectedSubclass}
                onSelect={onSelect}
                onClose={() => {
                    // In levelup context, we don't want to allow closing without selection
                    // User must go back through the wizard or select a subclass
                }}
                onConfirm={onNext}
            />
        </div>
